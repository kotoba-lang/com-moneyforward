(ns moneyforward.main
  "Kotodama WASM entrypoint for the Moneyforward clean-room actor (L4) — Clojure port.
  ...
  facts (`moneyforward.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "moneyforward")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"moneyforward.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "moneyforward-compat" :tier tier :entities entities} 200])
...
(defn start! [] :moneyforward-compat/ready)
